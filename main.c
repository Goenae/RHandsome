...
int main(){
    ...
    // printf("\n\n\nencrypted AES : %s \n\n\n", debug_bytes(encrypted_aes_key,sizeof(encrypted_aes_key)));

    const unsigned char* encrypted_iv = encrypt_RSA(public_key_pem, iv, iv_size);
    const unsigned char* encrypted_iv_lisible = debug_bytes(encrypted_iv, 512);

    printf("\nencrypted iv : %s \n\n\n", debug_bytes(encrypted_iv, 512));

    printf("%d", strlen((char*)encrypted_iv));
    const unsigned char* decrypted_iv = decrypt_RSA("keys/private_key.pem", encrypted_iv_lisible, 512);

    // printf("%s", decrypted_iv);
